...
static UIImage* ExtractIcon(NSObject<FlutterPluginRegistrar>* registrar, NSArray* iconData) {
  ...
  } else if([iconData.firstObject isEqualToString:@"fromCache"]) {
    if (iconData.count == 2) {
      @try {
        NSNumber* index = [NSNumber numberWithInteger:ToInt(iconData[1])];
        image = [FLTGoogleMapsCache.cache objectForKey:index];
      } @catch (NSException* exception) {
        @throw [NSException exceptionWithName:@"InvalidByteDescriptor"
                                       reason:@"Unable to interpret bytes from cache as a valid image."
                                     userInfo:nil];
      }
    } else {
      NSString* error = [NSString
          stringWithFormat:@"fromCache should have exactly one argument, the bytes. Got: %lu",
                           (unsigned long)iconData.count];
      NSException* exception = [NSException exceptionWithName:@"InvalidByteDescriptor"
                                                       reason:error
                                                     userInfo:nil];
